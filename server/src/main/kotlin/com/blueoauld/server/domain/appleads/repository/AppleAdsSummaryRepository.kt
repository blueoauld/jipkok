...
interface AppleAdsSummaryRepository : JpaRepository<AppleAdsKeywordDaily, Long> {
    ...
    @Query(
        ...
        select k.keyword_id as keywordId,
               ...
               (array_agg(k.currency order by k.report_date desc) filter (where k.currency is not null))[1] as currency,
    ...
    @Query(
        ...
        select s.search_term as searchTerm,
               ...
               (array_agg(s.currency order by s.report_date desc) filter (where s.currency is not null))[1] as currency,
