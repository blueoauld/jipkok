...
export function AccessEnvironmentCard({ data }: Props) {
  ...
  const platformTotal = platforms.reduce(
    (sum, p) => sum + data.platforms[p],
    0,
  );
  const versionTotal = data.versions.reduce((sum, v) => sum + v.count, 0);
          ...
          <div className="flex h-2 overflow-hidden bg-neutral-200 dark:bg-neutral-800">
            {platforms.map((platform, index) => (
              <div
                key={platform}
                style={{
                  width: `${(data.platforms[platform] / platformTotal) * 100}%`,
          ...
          <div className="flex gap-4 text-sm">
            {platforms.map((platform, index) => (
              <span key={platform} className="flex items-center gap-1.5">
                ...
                {Math.round((data.platforms[platform] / platformTotal) * 100)}%)
              </span>
            ))}
          </div>
        </div>
            ...
            {data.versions.map((row) => (
              <TableRow key={`${row.version}-${row.platform}`}>
                ...
                <TableCell className="text-right tabular-nums text-muted-foreground">
                  {Math.round((row.count / versionTotal) * 100)}%
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
